...
- Детально разбирается в сервисе/модуле/компоненте.
- Может ответить (или найти ответы) на любой вопрос, связанный с работой конкретного сервиса/модуля/компоненты.
- Несет ответственность за стабильность работы сервиса/модуля/компоненты.
- Разбирает user story и превращает хотелку продакта/пользователя в техническую задачу.
- Разрабатывает сервис/модуль/компоненту под ключ.
- Следит за отсутствием "узких мест" в сервисе/модуле/компоненте.
...
- Продумывает архитектуру сервиса/модуля/компоненты и умеет обосновывать принятые решения.
- Самостоятельно подготавливает архитектуру разрабатываемых компонент к защите перед командой.
- Отвечает за архитектуру нескольких сервисов/модулей/компонент.
- Принимает во внимание дальнейшее развитие всего сервиса/модуля/компоненты.
- Проектирует сложные системы, затрагивающие многие сервисы/модули/компоненты.
...
- Занимается стратегией и тактикой развития сервиcа/модуля/компоненты.
